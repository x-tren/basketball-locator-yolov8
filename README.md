...
### 模型规模对比：验证集更准的模型，在视频上反而更差

我另外训练了一个 **yolov8s** 作为对照（同为 416 分辨率、同为 COCO 预训练微调，
只改模型规模，其他完全一致），结果是个反直觉的发现。
...
|---|---|---|
| mAP50 | 0.9443 | **0.9629** |
| mAP50-95 | 0.5284 | **0.5560** |
| Recall | 0.870 | **0.913** |
| Precision | 0.958 | 0.951 |
| 训练耗时 | 26 分钟 | 2.42 小时（第 99 轮最佳，触发早停） |

**但在实际验收视频上，s 明显更差：**

| | yolov8n | yolov8s |
|---|---|---|
| 有检出的帧 | **176 / 176（100%）** | 157 / 176（89.2%） |
| 平均置信度 | **0.928** | 0.840 |
| top-1 框最大面积 | 58.2% | 66.3% |

逐帧排查后定位了原因：**s 对球场地面颜色的误检更激进**。

- **10 帧**（18, 19, 31, 32, 76, 77, 114, 115, 129, 130）里，s 只输出一个约 65% 的巨大地面框，
  完全没有报篮球 —— 被面积上限过滤后就什么都不剩了。
- **5 帧**（78, 97, 146, 147, 150）完全没有检测。
  把这 5 帧的置信度阈值降到 0.05 复查：球其实**大多仍被检测到**，
  只是置信度掉到 **0.09 ~ 0.34**（同一批帧上 nano 输出的是正常置信度）。

也就是说，s 多出来的那部分容量，没有用在「更好地识别这个带强运动模糊的近距离球」上，
反而**放大了训练集里本就存在的颜色混淆问题**。

**结论：最终交付使用 yolov8n。**

这也是本次任务里最值得记下的一条经验：验证集的 mAP 是在 416×416 的静态图上算出来的，
而验收视频是 4K 竖屏、球占画面比例大、运动模糊强，两者分布差异很大。
**在只有 600 张图的小数据集上，验证集指标不能直接外推到实际场景。**
如果当初只盯着 mAP50 从 0.944 涨到 0.963 就换成 s，交付的视频反而会退步。
...
| 参数 | 取值 | 理由 |
|---|---|---|
| 模型 | `yolov8n` | 实测在本视频上优于 yolov8s，理由见上方「模型规模对比」 |
